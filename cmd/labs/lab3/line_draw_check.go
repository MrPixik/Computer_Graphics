...
func drawPixelTest(file *os.File, p utils.Point) {
	text := fmt.Sprintf("(%d, %d)\n", p.X, p.Y)
	file.WriteString(text)
}

func bresenhamLineAlgorithmTest(file *os.File, p1, p2 utils.Point) {
	if p1.X > p2.X {
		p1, p2 = p2, p1
	}
	...
	if dx >= dy { // Линия ближе к горизонтальной
		...
		for i := 0; i <= dx; i++ {
			drawPixelTest(file, utils.Point{x, y}) // Рисуем текущий пиксель
...
func DrawlineTest() {
	...
	bresenhamLineAlgorithmTest(file, utils.Point{0, 0}, utils.Point{8, 3})

	file.WriteString("Линия от (8, 3) до (0, 0):\n")
	bresenhamLineAlgorithmTest(file, utils.Point{8, 3}, utils.Point{0, 0})
}
